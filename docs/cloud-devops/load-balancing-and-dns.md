...
The Application Load Balancer and Network Load Balancer in depth — routing, health checks, TLS termination via ACM — plus Route 53 DNS records and routing policies. Heavy on the actual `aws elbv2`/`aws acm`/`aws route53` CLI workflows and the failure modes (502s, 503s, timeouts) you actually hit wiring these together, not just the conceptual comparison.

## Application Load Balancer (ALB) Deep Dive

- The ALB operates at Layer 7 (HTTP/HTTPS).
- Think of it as a smart receptionist — every request arrives at the front desk, the receptionist checks what's needed (path, host, headers) and sends the guest to the right team (target group).
- It also checks which team members are healthy — if someone failed their health check, no traffic goes their way.
...
Internet
   │
   │  HTTPS :443 — TLS terminates here (cert from ACM, see below)
   ▼
...
- Standard three-tier VPC pattern — a public tier for the ALB, a private tier for the app instances, a private tier for the database — the ALB is the only new resource sitting in the public tier.
- Everything below it, including the instances it forwards to, stays in private subnets with no route to the internet.
...
- One ALB replaces what would otherwise be multiple Nginx reverse proxy configurations.
- You define the routing in the AWS console or CLI, not in config files.

#### ALB + target group for an ECS Fargate service, from the console

- The fastest way to see an ALB actually work is through the console, before ever touching the CLI — build it by hand first, then automate.
- This version targets an ECS Fargate service; the CLI-based lab right below covers the EC2-instance-target version in full, with the exact same underlying concepts, just a different registration mechanism.
- Three parts, done through the console, in this order, since each one needs the previous to already exist.
...
!!! note "Only the target type actually changes"
    ...
    - Everything else about the ALB — the listener, the health check states, the ALB-vs-NLB distinction, weighted target groups for canary — is identical to the EC2-instance version covered next.
    - The one real difference is registration: an EC2 target group is registered manually (or by an ASG on launch), while a Fargate service registers and deregisters its own IP targets automatically as tasks start and stop.
...
## Health Checks — Debugging Guide

- The ALB periodically sends an HTTP request to each target.
- If the response is healthy, traffic flows; if unhealthy, the target is removed from rotation.
...
- The ALB sent the health check but got **no response at all** — packets were silently dropped.
- Almost always a security group issue.
...
- Health check passes but browser shows 502.
- This means the health check port and traffic port are different:
...
# Empty = no targets registered → attach ASG + register instances (see the ALB lab above)
...
- **ACM (AWS Certificate Manager)** provides free, auto-renewing TLS/SSL certificates.
- The ALB terminates TLS — meaning it handles encryption/decryption, and forwards plain HTTP to your backend instances.
...
- Your EC2 instances don't need to handle TLS certificates, reducing complexity and CPU overhead.
- The certificate auto-renews — no more expired cert emergencies.
...
- It doesn't inspect HTTP headers or paths — it just forwards TCP connections extremely fast.
...
- Use ALB for everything HTTP.
- NLB is for when you need TCP pass-through (database proxies, gRPC, gaming servers) or static IPs for firewall allowlisting.
...
!!! success "Use Alias"
    ...
    - Alias records are free (no query charge).
    - They support zone apex (`example.com`, not just subdomains).
    - They automatically resolve to the current IP of the AWS resource.
...
- Time-to-live controls how long resolvers cache a record before re-checking.
- Low TTL (60s) means changes propagate fast but you pay more DNS query volume and clients feel any Route 53 blip.
- High TTL (3600s+) is cheaper and steadier but a bad record takes longer to fix everywhere.
- **Drop the TTL to 60 seconds before a planned cutover** (like switching to a new ALB), then raise it back afterward.
