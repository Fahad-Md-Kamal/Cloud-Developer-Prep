---
title: "Databases: RDS Operations"
---

# Databases: RDS Operations

AWS RDS operations — Multi-AZ failover vs read replicas, automated backups and point-in-time recovery, parameter groups, and safe patterns for running destructive SQL and EXPLAIN-driven query diagnosis against a production RDS instance. This complements the site's separate PostgreSQL for Scale page, which covers indexing, partitioning, and replication at the database-engine level — this page stays at the AWS operations layer: what you configure and run through the `aws rds` CLI and the AWS side of failover, backup, and connectivity.
...
- If the primary needs replacing permanently (not a failover — a deliberate migration), a replica can be promoted to a standalone, writable instance.
- This is one-way — you can't turn it back into a replica afterward.
...
- Measured in seconds.
- A steadily growing number means the replica can't keep up with write volume — usually a sign to upsize the replica's instance class, not just tolerate the lag.
...
- Database engine configuration — PostgreSQL settings like `max_connections`, `shared_buffers`, `work_mem`.
- Applied to an instance or cluster.
...
- Test the reverse script **before** you run the forward migration.
- If you can't undo it cleanly, that changes your deployment plan (you need a longer bake time before committing).

#### Connecting to a private RDS instance

- Your database has no public IP — `psql` from your laptop can't reach it directly.
- Tunnel through an instance inside the VPC using Session Manager, no bastion or open SSH port needed:
...
- A single `UPDATE` or `DELETE` touching millions of rows holds locks for the whole duration and can bloat the transaction log.
- Batch it instead:
...
!!! note "Maintenance window awareness"
    ...
    Schedule large batch operations outside RDS's maintenance window and outside your backup window (see above) — a patch, a backup, and a million-row batch job all competing for I/O at once is how a routine migration turns into an incident.
...
- Plans nest — the innermost (most indented) lines run first, feeding rows up to the operations above them.
- Two costs matter per node:
...
- `shared hit` = read from cache (fast); `shared read` = read from disk (slow).
- A query that's fast in your test but slow in production is often the same query hitting a cold cache instead of a warm one.
- `BUFFERS` is how you tell the difference from the plan alone, without guessing.
