...
AWS's global infrastructure model, the shared responsibility model, service quotas and tagging/budget hygiene, and IAM in depth — policy documents, roles vs users, trust policies, permission boundaries, ARNs, and condition keys, with three real least-privilege IAM roles worked through end to end. This is account-and-access-layer material, distinct from what runs inside the account (see load-balancing-and-dns.md and databases-rds-operations.md for the compute/data side).

## AWS Global Infrastructure

#### Regions

- A region (e.g. `ap-south-1` for Mumbai, `us-east-1` for N. Virginia) is a cluster of data centres in a geographic area.
- Your data stays in the region you choose unless you explicitly replicate it.
- Not every region has every service, and pricing varies.

- For the ramp plan: `ap-south-1` (Mumbai) is close to Dhaka, has all major services, and is cheaper than US regions.
- `us-east-1` gets new services first and has the most documentation.

#### Availability Zones

- Each region has 2–6 AZs — physically separate buildings with independent power, cooling, and networking, connected by low-latency (<1ms) redundant fiber.
- If AZ-a loses power, AZ-b and AZ-c keep serving.
...
- Why this matters: when an EC2 instance in AZ-a dies and Auto Scaling launches a replacement in AZ-b, the old EBS volume is stranded.
- Stateful data should go in RDS or S3 (regional services), not local EBS.
...
- If your RDS database gets breached because the password was `admin123`, that's on you.
- If the underlying hardware fails, that's on AWS.
...
- `export AWS_PROFILE=sandbox` only applies to the **current shell session** — every `aws` command you run afterward in that same terminal uses it automatically, with no `--profile` flag needed.
- Close the terminal or open a new tab, and it's gone; that new session falls back to the CLI's default profile again.
...
- **Identity-based policies** attach to a user, group, or role — "What can this identity do?"
- **Resource-based policies** attach to a resource (S3 bucket policy, SQS queue policy) — "Who can access this resource?" They include a `Principal` field.
- AWS evaluates both together.
- For cross-account access you typically need both: a resource policy on the target allowing the external account, and an identity policy on the external role allowing the action.
...
- This says: "ECS tasks can assume this role." No human, no Lambda, no EC2 can.
- The **permission policy** is "What can this role do once assumed?" — the standard document with Actions and Resources.
- For cross-account, the trust policy says `"Principal": { "AWS": "arn:aws:iam::OTHER_ACCOUNT:role/auditor" }` — the auditor role from the other account can assume this role.
...
- The instance gets temporary credentials by calling the Instance Metadata Service.
...
## The Three IAM Roles for Assignment A1
...
!!! success "Justification"
    ...
    - The app reads assets from S3, reads database credentials from Secrets Manager, and writes its own logs.
    - It does NOT get PutObject, DeleteObject, any IAM actions, or any EC2/infra actions.
    - If this container is compromised, the attacker can read assets and secrets for this one service — they cannot modify infrastructure, delete data, or escalate privileges.
...
!!! success "Justification"
    ...
    - Operators deploy (update ECS services, register task definitions), manage instances, and rotate secrets.
    - The explicit Deny on `iam:*` prevents operators from modifying permissions.
    - The region condition prevents accidental cross-region operations.
...
!!! success "Justification"
    ...
    - Auditors see everything (logs, configs, IAM, CloudTrail) to verify compliance.
    - Explicit Deny on all mutations ensures that even if someone attaches an additional policy, the role still can't modify anything.
...
Remember the IAM policies from the [IAM section](#iam-identity-and-access-management) above? The `Resource` field uses ARNs to say exactly *which* thing the policy applies to:
...
## Condition Keys in Practice — Restricting by Location
...
Every distinct `Action` string used across the three IAM roles above, grouped by service — what each one does, and whether it shows up as an Allow or an explicit Deny.

| Service | Action | What it does | Effect | Used in |
|---|---|---|---|---|
| S3 | `s3:GetObject` | Read an object | Allow | [IAM](#iam-identity-and-access-management), [Role 1](#role-1-application-role-ecs-tasks) |
| S3 | `s3:ListBucket` | List a bucket's contents | Allow | [IAM](#iam-identity-and-access-management), [Role 3](#role-3-read-only-auditor) |
| S3 | `s3:PutObject` | Write an object | Deny | [Role 3](#role-3-read-only-auditor) |
| S3 | `s3:DeleteObject` | Delete an object | Deny | [Role 3](#role-3-read-only-auditor) |
| Secrets Manager | `secretsmanager:GetSecretValue` | Read a secret | Allow | [Role 1](#role-1-application-role-ecs-tasks), [Role 2](#role-2-operator-role-human-engineers) |
| Secrets Manager | `secretsmanager:PutSecretValue` | Write / rotate a secret | Allow | [Role 2](#role-2-operator-role-human-engineers) |
| CloudWatch Logs | `logs:CreateLogStream` | Open a new log stream | Allow | [Role 1](#role-1-application-role-ecs-tasks) |
| CloudWatch Logs | `logs:PutLogEvents` | Write log lines | Allow | [Role 1](#role-1-application-role-ecs-tasks) |
| CloudWatch Logs | `logs:GetLogEvents` | Read log lines | Allow | [Role 3](#role-3-read-only-auditor) |
| CloudWatch Logs | `logs:DescribeLogGroups` | List log groups | Allow | [Role 3](#role-3-read-only-auditor) |
| EC2 | `ec2:Describe*` | Any read-only Describe call | Allow | [Role 2](#role-2-operator-role-human-engineers), [Role 3](#role-3-read-only-auditor) |
| EC2 | `ec2:StartInstances` | Start a stopped instance | Allow | [Role 2](#role-2-operator-role-human-engineers) |
| EC2 | `ec2:StopInstances` | Stop a running instance | Allow | [Role 2](#role-2-operator-role-human-engineers) |
| EC2 | `ec2:Run*` | Launch new instances (RunInstances) | Deny | [Role 3](#role-3-read-only-auditor) |
| EC2 | `ec2:Terminate*` | Terminate instances | Deny | [Role 3](#role-3-read-only-auditor) |
| EC2 | `ec2:Create*` | Create EC2 resources (volumes, snapshots, etc.) | Deny | [Role 3](#role-3-read-only-auditor) |
| EC2 | `ec2:Delete*` | Delete EC2 resources | Deny | [Role 3](#role-3-read-only-auditor) |
| ECS | `ecs:Describe*` | Any read-only Describe call | Allow | [Role 3](#role-3-read-only-auditor) |
| ECS | `ecs:List*` | Any read-only List call | Allow | [Role 3](#role-3-read-only-auditor) |
| ECS | `ecs:DescribeServices` | Read service status | Allow | [Role 2](#role-2-operator-role-human-engineers) |
| ECS | `ecs:UpdateService` | Roll out a new task definition to a service | Allow | [Role 2](#role-2-operator-role-human-engineers) |
| ECS | `ecs:RegisterTaskDefinition` | Register a new task definition revision | Allow | [Role 2](#role-2-operator-role-human-engineers) |
| IAM | `iam:*` | Every IAM action, no exceptions | Deny | [Role 2](#role-2-operator-role-human-engineers) |
| IAM | `iam:Get*` | Read a single IAM resource | Allow | [Role 3](#role-3-read-only-auditor) |
| IAM | `iam:List*` | List IAM resources | Allow | [Role 3](#role-3-read-only-auditor) |
| IAM | `iam:Create*` | Create IAM resources (users, roles, policies) | Deny | [Role 3](#role-3-read-only-auditor) |
| IAM | `iam:Delete*` | Delete IAM resources | Deny | [Role 3](#role-3-read-only-auditor) |
| IAM | `iam:Put*` | Write inline policies / config | Deny | [Role 3](#role-3-read-only-auditor) |
| IAM | `iam:Attach*` | Attach a managed policy to a principal | Deny | [Role 3](#role-3-read-only-auditor) |
| RDS | `rds:Describe*` | Any read-only Describe call | Allow | [Role 3](#role-3-read-only-auditor) |
| CloudTrail | `cloudtrail:LookupEvents` | Search the audit log | Allow | [Role 3](#role-3-read-only-auditor) |
| STS | `sts:AssumeRole` | Assume a role and get temporary credentials | Allow | [Trust policies](#trust-policies-vs-permission-policies) |
