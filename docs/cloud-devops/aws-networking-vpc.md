---
title: "AWS Networking: VPC & Subnet Design"
---

# AWS Networking: VPC & Subnet Design

A hand-built VPC lab, worked from first principles: what a VPC actually is, the
bit-math behind CIDR notation, why subnets split into public and private, and
how route tables decide where traffic goes. Two full CIDR practice sets are
included further down — work through them on paper before checking the
answers, since mid-octet math is the part that trips people up in interviews.
Continues in [VPC Security, Gateways & the Hands-On Build](vpc-security-groups-and-flow-logs.md),
which covers Internet/NAT gateways, security groups vs. NACLs, flow logs, and
the full step-by-step build of this VPC.

## What a VPC Is

- A VPC (Virtual Private Cloud) is your own isolated network within an AWS
  region — think of it as your private data centre network in the cloud.
- Nothing can enter or leave your VPC unless you explicitly allow it.
- When you create a VPC, you define a CIDR block — the total pool of private
  IP addresses available to everything inside it.
...
<text style="font-family:'IBM Plex Sans',sans-serif;font-size:11px;fill:#5c5468" text-anchor="middle" x="240" y="314">The real build uses 3 tiers × 2 AZs = 6 subnets.</text>
<text style="font-family:'IBM Plex Sans',sans-serif;font-size:11px;fill:#5c5468" text-anchor="middle" x="240" y="328">See the full walkthrough for the complete picture.</text>
</svg>

## CIDR Planning on Paper

CIDR (Classless Inter-Domain Routing) notation defines a range of IP
addresses. Before the mental math, here's what's actually happening under
the hood.
...
- Each of the 4 numbers (called octets) is 8 bits; 4 × 8 = **32 bits total**.
- The range of each octet is 0–255 because 8 bits can represent 2⁸ = 256
  values (`00000000` to `11111111`).

### The `/` number = how many bits are locked

- The number after `/` tells you **how many bits from the left are fixed
  (the network part)**.
- The remaining bits are yours to use for individual addresses (the host
  part).
...
Each free bit can be either 0 or 1 — two choices. With multiple free bits,
you multiply the choices together:
...
- Think of it like a combination lock: a 4-digit lock where each digit is
  0–9 has 10 × 10 × 10 × 10 = 10,000 combinations.
- Same idea here — but each "digit" is a bit (only 0 or 1), so it's 2
  instead of 10, multiplied by itself for each free bit.
...
24 bits are locked = the first 3 octets (`10.0.1`) never change. The last
octet (8 bits) is free, so it ranges from 0 to 255:
...
16 bits locked = first 2 octets (`10.0`) never change. The last 2 octets
are free:
...
`10.0.0.0/28` means 28 bits are locked — that's 3 full octets (24 bits)
plus 4 more bits into the last octet. Only 4 bits are free:
...
Every CIDR from /16 to /32. The "third octet range" column is for
mid-octet splits — the tricky ones. Pin this somewhere visible.
...
### How This Applies to a Real VPC Layout
...
- The VPC is /16 (a big pool), and you carve it into /24 subnets (smaller
  pools) — like dividing a large office floor into rooms: the floor is the
  VPC, each room is a subnet.
...
## Subnet Types: Public vs Private
...
Every subnet is associated with a route table that determines where
traffic goes.
...
- Public subnets route to the Internet Gateway — direct and bidirectional.
- Private subnets route through a NAT Gateway — outbound only, the internet
  can't initiate connections in.

See [VPC Security, Gateways & the Hands-On Build](vpc-security-groups-and-flow-logs.md)
for the Internet Gateway, NAT Gateway, security groups, flow logs, and the
full step-by-step build of this VPC (`fahad-devops-vpc`), including how EC2
instances in these subnets are launched through an
[Auto Scaling Group](auto-scaling-groups.md) across both AZs.
