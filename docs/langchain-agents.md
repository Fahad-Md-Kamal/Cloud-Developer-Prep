...
FastAPI patterns (dependency injection, background tasks, connection
pooling) that apply regardless of what's behind the endpoint, see
[FastAPI](fastapi.md) — this page only covers what's specific to serving
a LangChain agent.
...
[FastAPI](fastapi.md); none of that is specific to serving an agent. What
