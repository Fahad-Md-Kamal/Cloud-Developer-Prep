...
!!! warning "Private content — repo must stay private"
    ...
    client case study (Appendix A). Merged into the same
    `Cloud-Developer-Prep` site/build as the genericized public content
    for convenience — this only stays safe as long as the whole repo is
